# Khosoverso — Supremacia do Protesto

Site **estático** (HTML/CSS/JS puro, sem servidor nem banco) dos Khosōs criados pelo
...
- A **Khosōdex** lista todos os Khosōs do catálogo do mestre, mas como cards
  **escuros/"???"** até serem descobertos.
- O mestre distribui um **JSON de liberados**; cada jogador **importa** esse arquivo na
  Khosōdex para revelar os Khosōs (igual ao importar/exportar da Ficha de RPG).
...
templates/              → demais páginas (khosodex, equipe, colecao, social, perfil)
data/                   → catálogo do mestre
  khosodex.json         → TODOS os Khosōs (o mestre preenche)
  liberados.exemplo.json→ modelo do JSON de liberados
  COMO_ADICIONAR_KHOSO.md → guia do mestre
statics/css/            → estilos
statics/js/             → elementos.js, catalogo.js, menu.js + JS por página
statics/uploads/khoso   → imagens dos Khosōs
```
...
O mestre edita `data/khosodex.json` e coloca as imagens em `statics/uploads/khoso/`.
Passo a passo em [`data/COMO_ADICIONAR_KHOSO.md`](data/COMO_ADICIONAR_KHOSO.md).
